...
func (ro *runOnce) Run(s gbt.StrategyHandler) (bool, error) {
	if ro.hasRun {
		return false, nil
	}
...
func (rp *runPeriod) getNow(s gbt.StrategyHandler) time.Time {
...
func (rp *runPeriod) getDateToCompare(s gbt.StrategyHandler) (time.Time, bool) {
	data, ok := s.Data()
